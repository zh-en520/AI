...
model = nb.GaussianNB()
#进行交叉验证
s = ms.cross_val_score(model,train_x,train_y,cv=5,scoring='accuracy')
print(s.mean())

s = ms.cross_val_score(model,train_x,train_y,cv=5,scoring='precision_weighted')
print(s.mean())

s = ms.cross_val_score(model,train_x,train_y,cv=5,scoring='recall_weighted')
print(s.mean())

s = ms.cross_val_score(model,train_x,train_y,cv=5,scoring='f1_weighted')
print(s.mean())
...
mp.pcolormesh(grid_x, grid_y, grid_z,
    cmap='gray')
mp.scatter(test_x[:,0], test_x[:,1], s=60, c=test_y,
